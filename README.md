...
Our project is made of two subprojects : meshconverter and igasolver. igasolver depends of meshconverter.
The project produces 2 libraries (libmeshconverter.so and libigasolver.so) and 2 binaries linked against their
respective library (projArunner and projBrunner).
Packages for multiple platform should be produced : 1 single for igasolver and 3 different for meshconverter
(libs, devel, doc).
...
When compiling igasolver independently we must have installed meshconverter before, either from source or
from a package.
...
## Project structure and organisation
    ...
    │   │   └── meshconverterConfig.cmake    # for subprojects interdependencies
    │   ├── CMakeLists.txt           # main CmakeLists.txt for meshconverter
    │   ├── doc
    │   │   └── <Doxygen files and CMakeLists.txt>
    │   ├── include
    │   │   ├── CMakeLists.txt
    │   │   └── meshconverter                # project subfolder to appear in includes (e.g. include <meshconverter/World.h>)
    │   │       ├── CMakeLists.txt
    ...
    │   └── test
    │       ├── CMakeLists.txt
    │       └── testmeshconverter.cxx
    ├── igasolver
    │   └── <same as meshconverter>
    └── README

The headers and source files are split in each subproject. The headers are under a directory with
the name of the project. We want that users of our libraries write "include <meshconverter/xxx.h>" and not
...
Important note : to be able to compile igasolver alone, one must first make and install meshconverter. 
Moreover, one must add to PATH and LD_LIBRARY_PATH the installation directory of meshconverter.
